...
Next, we conducted covariate checks to check the distribution of covariates between our treatment and control groups. Table 2 shows the output of the treatment assignment regressed on the covariates used in our models. Covariates showing a significant impact on whether the subject received treatment or control could indicate a potential covariate imbalance. 
...
```{r hhi analysis, include = FALSE, warning = FALSE}
# d %>% filter(
#   hhi > 0
# ) %>% ggplot(aes(x = hhi)) + geom_histogram() + theme_minimal() +
#   labs(title = 'Distribution of household income among survey respondents',
#        x = 'Household income factor level') 

median_hhi <- d[d[, hhi > 0]]$hhi %>% median()
mean_hhi <- d[d[, hhi > 0]]$hhi %>% mean()
```


We further checked how representative our sample was of the entire population. For instance, we observed that 52% of our respondents were female, while 48% were male, which is slightly off from the national average but does not lead us to be too concerned. However, we found that low-income households were overrepresented among our survey respondents. Our household income variable was reported as a factor variable with 25 different levels with a median ranging between \$30,000 and \$35,000. According to the Congressional Research Service, the median household income in the U.S. in 2019 was $68,700.[^9] This implies that our sample respondents more frequently had lower household incomes than would be expected on the national level, making our sample not representative of the broader U.S. population. While we were still able to draw conclusions from the data collected, we kept in mind the limitations of the generalizability of our data.

[^9]: The U.S. Income Distribution: Trends and Issues. Congressional Research Service, 13 Jan. 2021, https://sgp.fas.org/crs/misc/R44705.pdf. 
